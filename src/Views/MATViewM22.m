...
- (void)update{
  ...
  for(int y=0; y<2; y++){
    for(int x=0; x<2; x++){
      [textfields[y*2+x] setStringValue:[NSString stringWithFormat:@"%f", values[x*2+y]]];
  ...
  case MAT_CODE_STYLE_C_ROW_FIRST_1D:
    [codeText setStringValue:[NSString stringWithFormat:@"{%f, %f, %f, %f}", values[0], values[2], values[1], values[3]]];
    break;
