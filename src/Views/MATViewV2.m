...
- (void)awakeFromNib{
  ...
  #ifndef NDEBUG
    if(([self bounds].size.width != MAT_VIEW_WIDTH_1) || ([self bounds].size.height != MAT_VIEW_HEIGHT_2))
      printf("MATViewV2 view should have size %d x %d\n", MAT_VIEW_WIDTH_1, MAT_VIEW_HEIGHT_2);
  #endif
...
- (void)dealloc{
  for(int i=0; i<2; i++){[textfields[i] release];}
  [super dealloc];
}
...
- (void)update{
  ...
  case MAT_CODE_STYLE_C_COLUMN_FIRST_2D:
    [codeText setStringValue:[NSString stringWithFormat:@"{%f, %f}", values[0], values[1]]];
    break;
...
- (void)codeChanged:(NSTextField*)sender{
  ...
  if(!naEqualV2d(values, newvalues)){
    for(int i=0; i<2; i++){
